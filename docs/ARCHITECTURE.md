...
- Identity Verification and Video KYC PDF reports are stored in MinIO and referenced from `identity_checks.pdf_report` and `video_kyc_checks.pdf_report` by object key
...
| `document_extractions` | One row per Scan Document or Bulk Scan extraction; stores typed extracted fields (salary, marks, name, UID, etc.); `file_name` stores the uploaded filename and is the per-entity UPSERT key; `source_screen` is limited to `scan_document` or `bulk_scan`. Identity Verification and Video KYC do not write into this table. For bulk scans, the row is **always** written on save — deterministic OCR/layout parsers now run first for layout-heavy documents such as marksheets, while Gemma4 is kept as a normalisation/fallback layer when the OCR text is incomplete or the layout family is unknown. A forensics-summary stub (`_extraction_unavailable: true`) is still used when Ollama is offline and no deterministic parser can produce structured data. The `_has_gemma4_data` gate uses `bool(_bulk_ext)` not a key-count heuristic, so even all-null Gemma4 results are saved correctly. |
| `identity_checks` | One current row per entity-scoped Identity Verification result; stores match scores, Aadhaar/PAN payloads, MinIO object keys for the uploaded images, the evidence JSON, and the current PDF-report object key |
| `video_kyc_checks` | One current row per entity-scoped Video KYC result; stores liveness outcome, face-match outcome, identity-proof payload, address-proof payload, browser-location payload, address comparison fields, retained challenge snapshots, and the current PDF-report object key |
