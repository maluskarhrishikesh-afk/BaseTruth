...
class BaseTruthService:
    ...
    def scan_document(
        ...
        log.info(
            ...
            extra={
                ...
                "path": str(path),
                "size_bytes": _size,
                "size_kb": round(_size / 1024, 1),
                "suffix": path.suffix.lower(),
                "forced_entity_ref": forced_entity_ref or "(auto-detect)",
    ...
    def flag_entity_cases_for_review(
        self,
        entity_ref: str,
        reason: str,
        anomaly_types: List[str] | None = None,
    ) -> int:
        """Retroactively reopen auto-approved cases for an entity.

        Called after cross-document reconciliation detects anomalies that
        were invisible at individual-document scan time.

        Returns the number of cases reopened.
        """
        reopened = 0
        records = self._load_case_records()
        for case_key, rec in records.items():
            # Match cases belonging to this entity
            if f"::{entity_ref}" not in case_key:
                continue
            # Only reopen cases that were auto-approved (cleared by system)
            if rec.disposition not in ("cleared",):
                continue
            anomaly_label = ", ".join(anomaly_types[:3]) if anomaly_types else "cross-document anomaly"
            self.update_case(
                case_key,
                status="triage",
                disposition="open",
                priority="high",
                note_text=(
                    f"⚠️ Auto-reopened: {reason}\n"
                    f"Anomaly types: {anomaly_label}\n"
                    "This case was originally auto-approved but cross-document "
                    "reconciliation found inconsistencies that require human review."
                ),
                note_author="system",
            )
            reopened += 1
            log.info(
                "flag_entity_cases_for_review: case reopened",
                extra={"case_key": case_key, "entity_ref": entity_ref, "reason": reason},
            )
        return reopened
