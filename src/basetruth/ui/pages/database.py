...
_TABLE_SCHEMA: dict[str, list[tuple[str, str, str]]] = {
    ...
    "video_kyc_checks": [
        ...
        ("reference_doc_pic", "VARCHAR(500)", "MinIO key for reference identity document image"),
        ...
        ("current_address_text", "TEXT", "Human-readable address from reverse-geocoding GPS"),
        ("address_distance_meters", "FLOAT", "Distance in metres between address-proof and live GPS"),
        ("pdf_report", "VARCHAR(500)", "MinIO key for the generated PDF report"),
        ("identity_dtls", "JSONB", "Extracted fields from the reference identity document"),
        ("address_dtls", "JSONB", "Extracted fields from the address proof document"),
        ("current_location_json", "JSONB", "GPS fix captured during session {lat, lon, accuracy}"),
        ("challenge_snapshots_json", "JSONB", "Per-challenge frame snapshots and EAR values"),
